...
export default function App() {
  ...
  // O cliente só precisa do catálogo e das configurações da loja pra ver o cardápio.
  // Os pedidos (orders) só importam pro painel do admin, então não travam a tela do cliente.
  const menuReady = catalogLoaded && settingsLoaded;
  if (!menuReady) {
  ...
  if (view === "admin" && authed) {
    if (!ordersLoaded) {
      return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: CREAM, fontFamily: "'Quicksand', sans-serif", color: PURPLE_MID }}>Carregando painel...</div>;
    }
